...
use services::{
    spawn_calendar_reminder, spawn_daily_briefing, spawn_data_maintenance, spawn_history_backfill,
    new_schedule_status,
    ScheduleHandle, AnomalyWatcher, BatchAnalysisHandle, LiquidityJobHandle, MarketPollHandle,
    NewsPollHandle, ingest_poll, new_status_handle, IngestDeps,
};
use state::AppState;
...
async fn bootstrap(app: tauri::AppHandle) {
    ...
    let db = Arc::new(
        Database::open(&secrets.database_path).expect("db open"),
    );
    ...
    log::info!(
        "LLM providers: {:?}",
        llm.read().unwrap_or_else(|e| e.into_inner()).available_providers()
    );
    ...
    let anomaly = Arc::new(AnomalyWatcher::new(Arc::new(AnomalyDetector::new(anomaly_cfg))));
    let backfill_status = new_status_handle();
    let market_poll_slot: Arc<Mutex<Option<Arc<MarketPollHandle>>>> =
    ...
    if config.akshare_realtime_enabled && !config.watchlist.is_empty() {
    ...
    let scheduler = ScheduleHandle::start(
        app.clone(),
        state.clone(),
        &config,
        schedule_status,
