...
ThisIsMyQuant 采用本地桌面单体架构。Rust 负责期货与 A 股数据采集、模拟撮合、订单/持仓/资金计算、存储、分析编排、后台任务和 Tauri Commands；React 负责总览、市场发现、标的详情、自选、基础模拟盘、事件资讯、本地数据库、引用式 AI 和交互状态。
...
│   ├── market.rs             # 市场资产、排行榜、搜索、详情聚合
│   ├── watchlist.rs          # 自选分组、关注资产、备注和提醒
│   ├── events.rs             # 资讯、日历、公告、财报和异动事件聚合
│   ├── ai.rs                 # 引用式 AI 上下文、摘要任务和来源
...
│   ├── ai_context.rs         # AI 上下文 bundle 与来源约束
│   ├── liquidity.rs          # 流动性评分
...
├── components/
│   ├── AppShell.tsx          # 顶部导航、全局状态条、页面布局
│   ├── layout/               # PageShell、PageHeader、TopNav、DataQualityBadge
│   └── ui/                   # shadcn/ui 原子组件
├── features/
│   ├── chart/                # K 线与指标
│   ├── markets/              # 统一资产表、筛选、排行榜、迷你走势
│   ├── detail/               # 标的详情、K 线、事件、持仓、AI 摘要
│   ├── watchlist/            # 自选表、分组、备注、事件摘要
│   ├── events/               # 事件时间线、筛选、影响标的、AI 分析
│   ├── database/             # 数据域资产卡、同步、导出、清理
│   ├── ai/                   # AI 任务、来源列表、报告卡片
│   ├── analysis/             # 报告、流式分析、时间线
│   └── market/               # 旧行情组件，逐步迁移到 markets/detail
├── pages/
│   ├── OverviewPage.tsx      # 总览
│   ├── MarketsPage.tsx       # 市场发现
│   ├── AssetDetailPage.tsx   # 期货/A 股统一标的详情
│   ├── WatchlistPage.tsx     # 自选
...
│   ├── SettingsPage.tsx      # 设置
│   ├── TradingReviewPage.tsx # 交易复盘，设置页实验区入口
│   └── MarketReplayPage.tsx  # 回放训练，设置页实验区入口
...
```
用户/调度/事件/异动触发 → ai_context/analysis_runner → LLM Router
  → reports 入库 → analysis events → AI 页/报告页/详情页摘要
```

### 市场发现链路

```
SQLite + quote_cache + sectors + stock_data
  → get_market_overview/list_market_assets/get_market_leaderboard/search_assets
  → 市场页/标的详情/自选
```
...
```
Jin10/news_items/calendar/announcements/anomaly_watcher
  → list_market_events/get_event_summary
  → 事件资讯/详情页/AI 上下文
```
